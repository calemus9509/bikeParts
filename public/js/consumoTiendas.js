function leerEmpresas() {
    ...
    axios
        ...
        .then((res) => {
          ...
          <img src="${
              imagenesArray != null ? imagenesArray[0] : ""
          }" class="card-img-top" alt="..." style="object-fit: cover; border-radius: 15px 15px 0 0; height: 200px; width: 100%; object-position: center center;">
          <div class="card-body text-white bg-dark">
              <h5 class="card-title">${element.nombre}</h5>
              <p class="card-text">${element.nit}</p>
              <p class="card-text">${element.descripcion}</p>
              <a href="/pagina-principal" class="btn btn-dark" onclick="seleccionarEmpresa(${
                  element.id
              })" style="background-color: rgb(245, 90, 0);">Ver tienda</a>
          </div>
      </div>
      `;
